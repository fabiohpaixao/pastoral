 ...
 */
	protected function _scaffoldView(CakeRequest $request) {
		if ($this->controller->beforeScaffold('visualizar')) {
		...
		} elseif ($this->controller->scaffoldError('visualizar') === false) {
 ...
 * @throws NotFoundException
 */
	protected function _scaffoldSave(CakeRequest $request, $action = 'editar') {
		$formAction = 'edit';
		$success = __d('cake', 'atualizado');
		if ($action === 'adicionar') {
			$formAction = 'add';
	...
	protected function _scaffoldDelete(CakeRequest $request) {
		if ($this->controller->beforeScaffold('delete')) {
			...
			if ($this->ScaffoldModel->delete()) {
				$message = __d('cake', '%1$s foi %2$s excluido com sucesso', Inflector::humanize($this->modelClass), $id);
				return $this->_sendMessage($message);
			}
			$message = __d('cake',
				'Ocorreu um erro ao excluir %1$s %2$s',
 ...
 */
	protected function _sendMessage($message) {
		if ($this->_validSession) {
			$this->controller->Session->setFlash($message, 'Flash/sucesso');
		...
		if (isset($db)) {
			if (empty($this->scaffoldActions)) {
				$this->scaffoldActions = array(
					'index', 'list', 'visualizar', 'adicionar', 'create', 'editar', 'update', 'excluir'
				);
			} elseif (!empty($prefixes) && in_array($scaffoldPrefix, $prefixes)) {
				$this->scaffoldActions = array(
					$scaffoldPrefix . '_index',
					$scaffoldPrefix . '_list',
					$scaffoldPrefix . '_visualizar',
					$scaffoldPrefix . '_adicionar',
					$scaffoldPrefix . '_create',
					$scaffoldPrefix . '_editar',
					$scaffoldPrefix . '_update',
					$scaffoldPrefix . '_excluir'
				);
			}
			...
			if (in_array($request->params['action'], $this->scaffoldActions)) {
				...
				switch ($request->params['action']) {
					...
					case 'adicionar':
					case 'create':
						$this->_scaffoldSave($request, 'adicionar');
						break;
					case 'editar':
					case 'update':
						$this->_scaffoldSave($request, 'editar');
						break;
					case 'excluir':
